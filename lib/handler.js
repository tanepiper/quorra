...
exports.handler = (route, options) => {
  ...
  if (options.layout) {
    if (typeof options.layout === 'function') {
      ...
      settings.layout = Path.normalize(Hoek.isAbsolutePath(settings.layout) ? settings.layout : Path.join(settings.relativeTo, settings.layout));
    }
  }
...
internals.prepare = (response, renderProps, next) => {
  ...
  const content = internals[response.source.renderMethod](<RoutingContext {...renderProps} createElement={createElement} />);
